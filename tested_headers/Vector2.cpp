...
{
    return Set(matrix.Elem(e2x2_MatrixCoordID::MAT2_11) * m_Coords[X_COORD] + matrix.Elem(e2x2_MatrixCoordID::MAT2_12) * m_Coords[Y_COORD],
...
{
    return Vector2(m_Coords[0] * scalar, m_Coords[1] * scalar);
}
...
{
    return (m_Coords[X_COORD] * m_Coords[X_COORD] + m_Coords[Y_COORD] * m_Coords[Y_COORD]);
}
...
{
    if (IsZero()) { // Vector2::Normalize: Attempting to normalize a zero-length
                    // vector!
        return *this;
    }
    ...
    Set(m_Coords[X_COORD] / oldLength, m_Coords[Y_COORD] / oldLength);
...
{
    return m_Coords[X_COORD] * vec.m_Coords[X_COORD] + m_Coords[Y_COORD] * vec.m_Coords[Y_COORD];
}
...
{
    Vector2 result;
    if (IsZero()) { // Vector2::Project: Attempting to project onto a zero-length
                    // vector!
        return *this;
    }
...
Vector2& Vector2::ApplyAngle(const double& angle)
{
    ...
    return Set(c * m_Coords[X_COORD] - s * m_Coords[Y_COORD],
